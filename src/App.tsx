...
import { Import, Plus, RadioTower, RefreshCcw, ScanSearch, Sparkles } from 'lucide-react';
import type {
  AppSettings,
...
function AppInner(): JSX.Element {
  ...
  useEffect(() => {
    ...
    root.dataset.accent = persisted.settings.accentPreset;
    root.dataset.motion = persisted.settings.animationsEnabled ? 'full' : 'reduced';
  }, [persisted.settings.theme, persisted.settings.accentPreset, persisted.settings.animationsEnabled]);
  ...
  const handleImport = useCallback(async (payload: ImportPayload) => {
    try {
      ...
      setPersisted((prev) => ({ ...prev, lastSelectedProfileId: profiles[0]?.id ?? prev.lastSelectedProfileId }));
  ...
  const labels = {
    ...
    statistics: t(language, 'tabStatistics'),
    logs: t(language, 'tabLogs')
  };
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
          ...
          <div className="workspace-grid">
            <section className="servers-pane panel-surface">
              <div className="page-head">
                <div>
                  <h1>Серверы</h1>
                  <p>Черно-оранжевый desktop-макет с быстрым выбором профиля, импортом и пингом.</p>
                </div>
              ...
              <div className="servers-toolbar">
                ...
                <button type="button" className="secondary-button" onClick={() => setSubscriptionOpen(true)}><RadioTower size={16} /> URL</button>
                ...
                <button type="button" className="primary-button" onClick={() => void handleQuickTest()}><Sparkles size={16} /> Быстрый тест</button>
              </div>
        ...
        {tab === 'statistics' && <TopStatusBar stats={quickStats} runtime={runtime} activeProtocolLabel={activeProtocolLabel} selectedProfile={selectedProfile} />}
        {tab === 'logs' && <HistoryPanel history={persisted.history} runtime={runtime} />}
      </main>
