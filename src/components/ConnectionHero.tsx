...
import { Globe, Play, Power, RefreshCcw, ShieldCheck } from 'lucide-react';
  ...
  return (
    <section className="connection-stage panel-surface">
      <div className="connection-stage__canvas">
        <motion.div
          className={`connection-orb ${isConnected ? 'is-connected' : ''}`}
          animate={isConnected ? { scale: [1, 1.02, 1] } : { scale: 1 }}
          transition={{ duration: 2.4, repeat: Infinity, ease: 'easeInOut' }}
        >
          <motion.div className="connection-orb__ring connection-orb__ring--outer" animate={{ rotate: 360 }} transition={{ duration: 16, repeat: Infinity, ease: 'linear' }} />
          <motion.div className="connection-orb__ring connection-orb__ring--middle" animate={{ rotate: -360 }} transition={{ duration: 12, repeat: Infinity, ease: 'linear' }} />
      ...
      <div className="connection-stage__footer">
        ...
        <div className="connection-stage__profile-name">{currentProfile?.name ?? 'Профиль не выбран'}</div>
        ...
        <div className="connection-stage__profile-host">{currentProfile ? `${currentProfile.server}:${currentProfile.port}` : 'Выберите сервер слева'}</div>
        <button type="button" className="primary-button connection-stage__test-button" onClick={onQuickTest}>
        ...
        <div className="mode-switch" aria-label="Connection mode">
          ...
          <button type="button" className="mode-switch__item" disabled title="TUN будет добавлен отдельным backend-режимом">TUN</button>
        </div>
      </div>
    </section>
  );
